...
import { ADMIN_SESSION_COOKIE, getCookieValue, isValidSessionCookieValue } from "@/lib/admin-session";
import { isRateLimited, requestIp } from "@/lib/rate-limit";
import { getEffectiveRole } from "@/lib/require-admin";

// Los 4 endpoints de /api/jobs/* quedan fuera del matcher del proxy porque Vercel
...
// admin (better-auth o, mientras dure la transición, la cookie del password gate
// de Fase 0) para no romper ese flujo.
export async function isJobRequestAuthorized(request: Request): Promise<boolean> {
  const secret = process.env.CRON_SECRET;
  if (secret && request.headers.get("authorization") === `Bearer ${secret}`) {
    return true;
  }
  if (isValidSessionCookieValue(getCookieValue(request, ADMIN_SESSION_COOKIE))) {
    return true;
  }
  return (await getEffectiveRole()) === "admin";
}
